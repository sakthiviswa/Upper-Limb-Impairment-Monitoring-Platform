...
> **Single backend:** the entire API (auth, users, doctors, messaging,
> notifications, appointments, and rehab session monitoring) is served by
> **one FastAPI application**, run with **one `uvicorn` process**, backed
> by **one database**. There is no separate Flask server anymore — earlier
> versions of this project ran two backends (Flask on port 5000 + FastAPI
> on port 8000); everything has been consolidated into the FastAPI app
> described below.
...
rehab-monitor/
├── backend/
│   ├── main.py                          # Single FastAPI entry point (auth + rehab)
│   ├── database.py                      # Shared SQLAlchemy engine + get_db dependency
│   ├── security.py                      # JWT + password hashing + get_current_user
│   ├── schemas_auth.py                  # Pydantic: register/login
│   ├── schemas_user.py                  # Pydantic: profile/messages/appointments/etc
│   ├── requirements.txt
│   ├── .env.example
│   ├── outputs/
...
│   │   ├── schemas.py                   # Pydantic request/response schemas (sessions)
│   │   ├── user.py                      # User model (auth + doctor profile fields)
│   │   ├── notification.py              # Notification model
│   │   ├── message.py                   # Conversation + Message models
│   │   ├── exercise_assignment.py       # ExerciseAssignment model
│   │   └── appointment.py               # Appointment model
│   ├── routers/
│   │   ├── auth.py                      # POST /register, /login
│   │   ├── users.py                     # profile, doctors, messages, appointments, dashboards
...
└── frontend/
    ├── index.html
    ├── vite.config.js                   # Proxies /api and /rehab → single FastAPI backend
    ├── package.json
    └── src/
        ├── main.jsx
        ├── App.jsx
        ...
        ├── utils/
        │   ├── api.js                   # Axios client (auth/users)
        │   ├── rehabApi.js              # Axios client (rehab sessions) — same backend
        │   └── angleUtils.js            # Angle math + session stats
        ├── components/                  # Dashboard panels, chat, appointments, etc.
        ├── dashboards/                  # Patient / Doctor / Admin dashboards
        └── rehab/
...
**Rehab (patients / sessions / reports)**
...
**Auth / users / doctors / messaging / appointments** *(same backend, JWT-protected unless noted)*
...
| GET/PUT | `/api/doctor/profile[/{id}]` | Doctor profile |
| GET | `/api/notifications` | List notifications |
| POST | `/api/doctor/accept-patient/{id}` | Accept patient |
| GET | `/api/messages/conversations` | List conversations |
| POST | `/api/messages/send` | Send message |
| GET/POST/DELETE | `/api/appointments` | List/book/cancel appointments |
| GET | `/api/patient/dashboard`, `/api/doctor/dashboard`, `/api/admin/dashboard` | Role dashboards |

Full interactive docs (all endpoints): **http://localhost:8000/docs**
