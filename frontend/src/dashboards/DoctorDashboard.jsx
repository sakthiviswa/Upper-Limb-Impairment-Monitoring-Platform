...
import DoctorVerificationPanel from '../components/DoctorVerificationPanel'
import {
  Card, StatCard, SectionHeader, Badge, DataTable, LoadingScreen, PageHeader,
} from '../components/ui'
...
function EmptyState({ icon: Icon, text }) {
  return (
    <Card style={{ padding: '2.5rem', textAlign: 'center' }}>
      ...
      <div className="text-secondary" style={{ fontWeight: 500 }}>{text}</div>
    </Card>
  )
}
...
function ProgressBar({ value, max }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div className="ui-progress" style={{ flex: 1, minWidth: 56 }}>
        <div className="ui-progress__bar" style={{ width: `${pct}%`, background: color }} />
      </div>
      <span className="text-caption" style={{ fontVariantNumeric: 'tabular-nums' }}>{pct}%</span>
    </div>
  )
}
  ...
  return (
    <DashboardLayout user={user} role="doctor" activeTab={tab} onTabChange={setTab} unreadMessages={unreadMessages} pendingAppts={pendingAppts}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {tab === 'verification' && (
          <>
            <PageHeader title="Doctor Verification" subtitle="Upload your documents and track verification status" />
        ...
        {tab === 'myprofile' && <ProfileSettings viewMode={true}  />}
      </div>
    </DashboardLayout>
  )
}
...
function HeaderBtn({ children, onClick, badge }) {
  return (
    <button onClick={onClick} className="ui-btn ui-btn--secondary ui-btn--sm" style={{ position: 'relative' }}>
      {children}
      {badge > 0 && (
        <span className="app-sidebar__badge" style={{ position: 'absolute', top: -4, right: -4 }}>
          {badge}
        </span>
      )}
    </button>
