...
/* ── Helpers ─────────────────────────────────────────────────────── */
const AVATAR_COLORS = ['#2563EB', '#0EA5E9', '#22C55E', '#F59E0B', '#EF4444', '#64748B']
...
function Avatar({ name, size = 40 }) {
  return (
    <div className="fp-avatar" style={{ width: size, height: size, fontSize: size * 0.33 }}>
      {initials(name)}
    </div>
  )
}
    ...
    <div className="fp-card" style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '16px', borderBottom: '1px solid var(--border-light)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div className="fp-header__icon" style={{ width: 36, height: 36 }}>
            ...
            {totalUnread > 0 && (
              <span className="fp-badge fp-badge--danger">{totalUnread}</span>
            )}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loadingConvs ? (
            <div className="fp-empty" style={{ padding: 32 }}>
              <div className="fp-spinner" />
              <div className="fp-empty__text">Loading…</div>
            </div>
          ) : filtered.length === 0 ? (
            <div className="fp-empty" style={{ padding: 32 }}>
              <div className="fp-empty__text">{search ? 'No results found.' : 'No conversations yet.'}</div>
            </div>
          ) : filtered.map(conv => {
            ...
            return (
              <div
                ...
                className={`fp-list-item ${isActive ? 'fp-list-item--active' : ''} ${hasUnread ? 'fp-list-item--unread' : ''}`}
                style={{ alignItems: 'center' }}
              >
                <div style={{ position: 'relative' }}>
                  <Avatar name={conv.otherName} size={40} />
                  {hasUnread && (
                    <span style={{ position: 'absolute', top: -1, right: -1, width: 10, height: 10, borderRadius: '50%', background: 'var(--danger)', border: '2px solid var(--bg-card)' }} />
                  )}
                </div>

                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 4 }}>
                    <span style={{ fontSize: 14, fontWeight: hasUnread ? 700 : 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {conv.otherName}
                    </span>
                    {conv.lastMessageAt && (
                  ...
                  <div style={{ fontSize: 12, color: hasUnread ? 'var(--text-secondary)' : 'var(--text-muted)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: hasUnread ? 600 : 400 }}>
                    {conv.lastMessage || 'No messages yet'}
                  </div>
                </div>

                {conv.unread > 0 && (
                  <span className="fp-badge fp-badge--brand">{conv.unread}</span>
                )}
              </div>
            )
          })}
      ...
      {/* ══ Right: Chat pane ════════════════════════════════════════ */}
      {!activeConv ? (
        <div className="fp-empty" style={{ flex: 1, background: 'var(--bg-app)' }}>
          <div className="fp-empty__icon">
            <MessageSquare size={28} strokeWidth={1.5} />
          </div>
          <div className="fp-empty__title">Your Messages</div>
          <div className="fp-empty__text">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-app)' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>{activeConv.otherName}</div>
            ...
            <div style={{ flex: 1, position: 'relative' }}>
              <textarea
                ...
                style={{
                  ...
                  minHeight: 44, maxHeight: 120, paddingRight: 16,
                }}
              />
            </div>
            <button
