...
───────────────────────────────────────────────────────────────────── */
const INTENSITY = {
  Low:    { color: '#22C55E', bg: 'rgba(34,197,94,0.1)',  border: 'rgba(34,197,94,0.25)', dot: '#22C55E' },
...
function Card({ children, style = {}, noPad }) {
  return (
    <div
      className={noPad ? 'fp-card' : 'fp-card'}
      style={{ padding: noPad ? 0 : '1.25rem', ...style }}
    >
      {children}
    </div>
  )
          ...
          <button
            ...
            style={{
              ...
              background: showMedia
                ...
                : 'var(--bg-card2)',
              color: showMedia
                ? (media?.type === 'video' ? '#2563EB' : '#0ea5e9')
                : 'var(--text-secondary)',
              border: `1px solid ${showMedia
                ? (media?.type === 'video' ? 'rgba(37,99,235,0.3)' : 'rgba(14,165,233,0.3)')
                : 'var(--border)'}`,
            }}
          >
            {showMedia
  ...
  return (
    <div className="fp-root">
      ...
      <div className="fp-header">
        ...
        <button type="button" onClick={load} className="fp-btn fp-btn--ghost">
      ...
      {!error && assignments.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[
              { label: 'Total Plans',     value: assignments.length,                                      Icon: ListChecks, color: '#2563EB' },
              { label: 'Total Exercises', value: assignments.reduce((a, x) => a + x.exercises.length, 0), Icon: Dumbbell,   color: '#22C55E' },
              {
                label: 'Latest Plan',
